...
#repnumvolcano=5		#default 5 or 6

tissuetype=1		# liver tissue = 1;  blood plasma = 0	# MODIFY for sample type
mousemodel=4		# model 1 = CCl4_HFD # model 2 = MC4R-KO_HFD # model 3 = CDDA_HFD # 4 = STAM  # 7 = liver interday
...
ws2.cell(row=1, column=1).value='Lipid'
...
while go==1:
	...
	if isocount>1:				# multiple isomers found (e.g., multiple PC(16:0_20:3) species found)
		...
		ws2.cell(row=w, column=1).value=slipid	# copy lipid name and RT without any splitting or modification
...
if tissuetype==1:
	...
	while go==1:
		...
		sni=0
		while sni<snum:
			caq=ws2.cell(row=r, column=sni+3).value
			if caq is None:
				caq=0
				csnum=csnum-1
			else:
				cbq=ws2.cell(row=r, column=sni+3+snum).value
				if cbq is None:
					cbq=0
					csnum=csnum-1
				else:
		...
		ws2.cell(row=r, column=3+len(negdataname)+2).value=clipid
		ws2.cell(row=r, column=3+len(negdataname)+3).value=amean    #write 
		ws2.cell(row=r, column=3+len(negdataname)+5).value=bmean    #write 
		if len(alist)<2:
			astd=''
		else:
		...
		ws2.cell(row=r, column=3+len(negdataname)+4).value=astd    #write 
		ws2.cell(row=r, column=3+len(negdataname)+6).value=bstd    #write 
		if amean==0:
			fcm=0
		else:
			fcm=bmean/amean
			ws2.cell(row=r, column=3+len(negdataname)+7).value=fcm    #write 
		#print(fcm)
		if fcm==0:
			lgfcm=0
		else:
			lgfcm=math.log2(fcm)
			ws2.cell(row=r, column=3+len(negdataname)+9).value=lgfcm    #write 
		...
		if lgfcm==0:
			...
			ws2.cell(row=r, column=3+len(negdataname)+8).value=p    #write 
			ws2.cell(row=r, column=3+len(negdataname)+12).value=t    #write 
			...
			if p<0.05:		# determine if fold change and p value significant
				if fcm>2:
					...
					sign=1
			if sign==1:
				ws2.cell(row=r, column=3+len(negdataname)+11).value=clgp    #write significant p values / with significant fold changes
			else:
				if clgp==0:
					ok=1
				else:
					ws2.cell(row=r, column=3+len(negdataname)+10).value=clgp    #write insignificant
			# end calc P value
			# Begin calculate adjusted P values according to Bonferroni, Benjamini-Hochberg and Abundance-Step-Down
			lplist.append(clipid)
			if amean>bmean:
	...
	ws2.cell(row=1, column=3+len(negdataname)+2).value='Lipid'
	ws2.cell(row=1, column=3+len(negdataname)+3).value='Mean [col C-H]'
	ws2.cell(row=1, column=3+len(negdataname)+4).value='Std. dev. [col C-H]'
	ws2.cell(row=1, column=3+len(negdataname)+5).value='Mean [col I-N]'
	ws2.cell(row=1, column=3+len(negdataname)+6).value='Std. dev. [col I-N]'
	ws2.cell(row=1, column=3+len(negdataname)+7).value='Fold change of mean'
	ws2.cell(row=1, column=3+len(negdataname)+8).value='P value'
	ws2.cell(row=1, column=3+len(negdataname)+9).value='log2 fold change'
	ws2.cell(row=1, column=3+len(negdataname)+10).value='="-log10 p-value (raw, not significant)"'
	ws2.cell(row=1, column=3+len(negdataname)+11).value='="-log10 p-value (raw, significant)"'
	ws2.cell(row=1, column=3+len(negdataname)+12).value='t-test statistic'
	# End calculate means, standard deviations, fold changes and p-values

	# Begin calculate adjusted P values according to Bonferroni, Benjamini-Hochberg and Abundance-Step-Down
	ws2.cell(row=1, column=3+len(negdataname)+13).value='P value (Bonferroni)'
	ws2.cell(row=1, column=3+len(negdataname)+14).value='P value (BH)'
	ws2.cell(row=1, column=3+len(negdataname)+15).value='P value (ASD)'
	ws2.cell(row=1, column=3+len(negdataname)+16).value='="-log10 p-value (Bonferroni)"'
	ws2.cell(row=1, column=3+len(negdataname)+17).value='="-log10 p-value (ASD)"'
	ws2.cell(row=1, column=3+len(negdataname)+18).value='="-log10 p-value (BH; insignificant)"'
	ws2.cell(row=1, column=3+len(negdataname)+19).value='="-log10 p-value (BH; also significant after ASD and BF)"'
	ws2.cell(row=1, column=3+len(negdataname)+20).value='="-log10 p-value (BH; also significant after ASD, but not BF)"'
	ws2.cell(row=1, column=3+len(negdataname)+21).value='="-log10 p-value (BH; significant, but not after ASD or BF)"'
	nmc=len(lplist)
	#print('Lists for BH:')
	...
	while apvi<len(lplist):
		if pvlist[apvi]==1:
			...
			logbfpv=(-1)*logbfpv
			ws2.cell(row=2+apvi, column=3+len(negdataname)+13).value=bfpv
			ws2.cell(row=2+apvi, column=3+len(negdataname)+16).value=logbfpv
			# end Bonferroni
			...
			logasdpv=(-1)*logasdpv
			ws2.cell(row=2+apvi, column=3+len(negdataname)+14).value=asdpv
			ws2.cell(row=2+apvi, column=3+len(negdataname)+17).value=logasdpv
			...
			logbhpv=(-1)*logbhpv
			ws2.cell(row=2+apvi, column=3+len(negdataname)+15).value=bhpv
			...
			if abs(lgfclist[apvi])<1:
				ws2.cell(row=2+apvi, column=3+len(negdataname)+18).value=logbhpv	# 1) Insignificant due to small fold change
			else:
				if bhpv>0.05:
					ws2.cell(row=2+apvi, column=3+len(negdataname)+18).value=logbhpv	# 1) Insignificant after BH
				else:
					if bfpv<0.05:
						ws2.cell(row=2+apvi, column=3+len(negdataname)+19).value=logbhpv	# 2) Significant after BH and significant after BF and ASD
					else:
						if asdpv<0.05:
							ws2.cell(row=2+apvi, column=3+len(negdataname)+20).value=logbhpv	# 3) Significant after BH and significant after ASD, but not after BF
						else:
							ws2.cell(row=2+apvi, column=3+len(negdataname)+21).value=logbhpv	# 4) Significant after BH, but not after ASD or BF 
