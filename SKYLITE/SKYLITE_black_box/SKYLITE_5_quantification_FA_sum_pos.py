...
if excelinput==1:
	...
	while rrow<58:
		...
		lcis=wsin2.cell(row=srow, column=1).value
		lcis=str(lcis)
		if int(cis)==0:
			if rrow<44:
				exui=rrow-2
			...
			while gosis==1:
				...
				lscis=str(lscis)
				if lcis[0]+lcis[1]+lcis[2]==lscis[0]+lscis[1]+lscis[2]:
					if scis==0:
						sli=sli+1
					else:
						# found substitute IS
...
if today in runidentifier:
	outfilename=runidentifier+'_SKYLITE_Q5_S6_neg.xlsx'
else:
	outfilename=today+runidentifier+'_SKYLITE_Q5_S6_neg.xlsx'
