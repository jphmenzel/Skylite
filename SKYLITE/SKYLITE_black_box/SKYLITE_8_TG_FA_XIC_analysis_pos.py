...
if insequence==0:
	...
	crepnamex=str(sys.argv[1]) 	# from argument
	if str(crepnamex[len(crepnamex)-1])=='0':
		repi=0
	else:
		repi=1
	#if str(crepnamex[len(crepnamex)-1])=='_':
	crepname=crepnamex[:-1]
	#elif str(crepnamex[len(crepnamex)-1])=='0':
	#	crepname=crepnamex[:-1]
	#else:
	#	crepname=crepnamex
	#if crepname=='FB1_':		# MODIFY TO TEST FOR FIRST SAMPLE IN QUEUE # NOT REQUIRED ANYMORE
	#	repi=0
	#else:
	#	repi=1
...
if convertfile==1:
	print('Begin conversion of chromatograms file from tsv to csv...')
	...
	if option==1:
		...
		tempdf=pd.read_csv('skyl_xic_report_1_intensities_pos.csv', header=None, skiprows=1, delimiter=',', names=list(range(cimax)), engine='python')
	elif option==3:
		tempdf=pd.read_csv('skyl_xic_report_1_intensities_pos.csv', header=None, skiprows=1, delimiter=',', names=list(range(cimax)), low_memory=False, lineterminator='\n')
...
if excelinput==1:
	print('Reading Skyline report from pos mode acquisition, sum composition to retrieve RT ranges for TGs.')
	# get file name
	wbin=openpyxl.load_workbook('SKYLITE_INPUT_sample_list.xlsx')
	ws3=wbin['Input_file_names']
	cfn=ws3.cell(row=21, column=6).value
	filenamerep5s8=str(cfn)+'.csv'
	print(filenamerep5s8)
	ntrdf=pd.read_csv(filenamerep5s8, low_memory=False)
...
if excelinput==1:
	...
	wbin=openpyxl.load_workbook('SKYLITE_INPUT_sample_list.xlsx')
	ws3=wbin['Input_file_names']
	cfn=ws3.cell(row=25, column=6).value
	qtgfilename=str(cfn)+'.xlsx'	#filenameq6s8
	print(qtgfilename)
else:
...
if insequence==0:
	...
	if crepname[len(crepname)-1]=='_':
		csheetname='TG-FA_'+crepname[:-1]
		csheetnamepr='FA_'+crepname[:-1]
	else:
		csheetname='TG-FA_'+crepname#[:-1]
		csheetnamepr='FA_'+crepname#[:-1]
	wstgfa=wbtgfa.create_sheet(csheetname)
