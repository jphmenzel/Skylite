...
if excelinput==1:
	...
	print('This program calculates quantities of lipids from curated Skyline report files (sum composition quantification, positive mode) of the SKYLITE workflow.')
	...
	while goread==1:
		...
		if csample is None:
			...
			ctt=wsin1.cell(row=rrow, column=3).value
	...
	if vdisplay==1:
		if internalstandard==1:
			print('Internal standard is Ultimate SPLASH ONE')
		else:
			print('Internal standard is the custom Inselspital Internal Standard Mix (IS-Mix)')
	...
	cfn=wsin3.cell(row=21, column=6).value
	...
	subislist=[]		# define, which Ultimate SPLASH ONE internal standards is to be used instead
	rrow=2
	srow=rrow+1
	while rrow<58:
		cis=wsin2.cell(row=rrow, column=2).value
		lcis=wsin2.cell(row=rrow, column=1).value
		lcis=str(lcis)
		if int(cis)==0:
			if rrow<44:
				exui=rrow-2
			...
			while gosis==1:
				srow=rrow+slist[sli]
				scis=wsin2.cell(row=srow, column=2).value
				lscis=wsin2.cell(row=srow, column=1).value
				lscis=str(lscis)
				if lcis[0]+lcis[1]+lcis[2]==lscis[0]+lscis[1]+lscis[2]:  # check for lipid class match 
					if scis==0:
						sli=sli+1
					else:
						# found substitute IS
	...
	ismvol=810*0.001*0.001 # L (1 uL)			volume of pre-prepared ISM solution used	#default is 810 uL ISM solution used for each sample
	...
	mousemodel=9 # 6 is NIST validation 	# 5 is all 4 together				# MODIFY TO SELECT WHICH VOLCANO PLOT IS TO BE MADE
	splashcheck=0 		# default is 0; set 1 to write into file internal standard input values instead of performing analysis
...
if internalstandard==2:
	...
	while ismi<len(ismname):
		cismn=(ismvol*ismc[ismi]*0.001)/ismmm[ismi]	#current n [mol] of USP standard used in sample
		ismn.append(cismn)
		ismi=ismi+1
...
if today in runidentifier:
	outfilename=runidentifier+'_SKYLITE_Q5_S6_pos.xlsx'
else:
	outfilename=today+runidentifier+'_SKYLITE_Q5_S6_pos.xlsx'
...
# begin write top row in output file
...
# end write top row in output file
...
while r<len(negreport[0]):
	if 'd5' in str(negreport[1][r]):
		...
		if str(negdataname[0]) in str(negreport[20][r]):
			...
			lplist.append(clipidname)		# Will be written in output excel file, column 1 (list of lipids)
			lpilist.append(count)			# Index in output excel file
			count=count+1
	r=r+1
...
while r<len(negreport[0]):
	if 'd5' in str(negreport[1][r]):
		...
		#	#print(negreport[20][r])
		if rnnf==0:
			# The current (r) row in negreport (Skyline report) is for a replicate in the replicate list (negdataname)
			if qcheck==2:
			...
			while k<len(negreport[0]):
				if str(negreport[1][k])==str(negreport[1][r]):
					ok=1
				elif negreport[4][k]==negreport[4][r]:	# found an isomer of the same sum composition (m/z), but different FAs
